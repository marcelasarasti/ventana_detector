...
while True:
    ...
    # CONVERTIR A HSV Y GRIS
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

    # DETECTAR LADRILLO POR COLOR NARANJA
    ...
    # DETECTAR VENTANA NEGRA POR COLOR (HSV)
    lower_black = np.array([0, 0, 0])
    upper_black = np.array([180, 255, 50])
    mask_window = cv2.inRange(hsv, lower_black, upper_black)

    # DILATACIÓN FUERTE PARA UNIR LAS REJAS
    ...
    for cnt in contours:
        ...
        area = w * h
        aspect_ratio = w / float(h)
        if (area > 2000 and 0.5 < aspect_ratio < 3.5 and
                w > 50 and h > 50):  # asegurarse de que no sea una franja
            window_candidates.append((x, y, w, h))

    if not window_candidates:
        continue

    xw, yw, ww, hw = max(window_candidates, key=lambda r: r[2] * r[3])
